...
from model.mpmOnQuadtree.hilbertCurve import *
...
def bottom_up(randomForestList, resolutionTranProb, prior, R, C, H, W, _method):
    ...
    for h in range(0, H):
        for w in range(0, W):
            ...
            for xs in range(0, C):
                if _method != "gauss":
                    partialPost[0][xs][h][w] = randomForestList[0][xs][h][w][0]
                else:
    ...
    for r in range(1, R):
        ...
        for h in range(0, height):
            for w in range(0, width):
                ...
                for xs in range(0, C):
                    if _method != "gauss":
                        partialPost[r][xs][h][w] = randomForestList[r][xs][h][w][0]
                    else:
...
def top_down(R, H, W, experiment, partialPost, transitionContrib, transitionContribSingle, partial_folder):
    ...
    for r in tqdm(reversed(range(R - 1))):  # complete top down
        ...
        if _version == "SMMRF":
            ...
            hilZZ(height, width, r, fx, fy, partialPost, alreadyChanged, transitionContribSingle)
        else:
